...
class Curl {
    const USER_AGENT = 'PHP-Curl-Class/1.0';
    ...
    protected function exec($_ch=null) {
        ...
        $ch->curlinfo = curl_getinfo($ch->curl);
        if (!$ch->error) {
...
$curl = new Curl();
// 支持毫秒级必须  CURL>=7.16.2, PHP>=5.2.3
$curl -> setOpt(CURLOPT_ENCODING, "gzip");
if(substr($url, 0, 5) == 'https'){
    $curl->setOpt(CURLOPT_SSL_VERIFYPEER, false); // 信任任何证书
    $curl->setOpt(CURLOPT_SSL_VERIFYHOST, false); // 检查证书中是否设置域名
}
$_total_request_times = 0;
while($_total_request_times <= 10){
    $starttime = microtime();
    $_total_request_times++;
    $response_code = $curl -> get($url);
    //$response_code = $curl -> post($url, $post_params);
    $this->_curlinfo = $curl -> curlinfo;
    if($response_code!== 0){
    }else{
        break;
    }
